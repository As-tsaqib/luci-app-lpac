...
return view.extend({
	...
	handleSaveConfig: function() {
		...
		if (!configChanged && !refreshChanged) {
		...
		return Promise.all(tasks.map(function(task) {
			return task.then(function() { return null; }, function(error) { return error; });
		})).then(function(errors) {
			ui.hideModal();
			const error = errors.find(function(value) { return value !== null; });
